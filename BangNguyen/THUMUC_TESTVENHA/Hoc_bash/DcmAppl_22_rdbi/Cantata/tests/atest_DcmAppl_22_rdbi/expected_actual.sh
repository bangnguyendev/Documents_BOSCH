...
do
	...
	then
		...
		then
			...
			array_actual=(`cat $file| sed -n "$temp_line,$i p" | grep -A 2 '>>  FAILED: Check:'| grep '           actual: ' |sed "s/[a-vG-Z:'<>]//g" |sed "s/ [A-Z]//g"`)
		...
		then
			...
			array_actual=(`cat $file| sed -n "$temp_line,$i p" | grep -A 4 '>>  FAILED: Check: ACCESS_VARIABLE'| grep '           actual: ' |sed "s/[a-vG-Z:'<>]//g" |sed "s/ [A-Z]//g"`)
			
			cout_array=0
			for dem_temp in `cat $file| sed -n "$temp_line,$i p" | grep -A 4 '>>  FAILED: Check: ACCESS_VARIABLE'| grep '           actual: ' |sed "s/[a-vG-Z:'<>]//g" |sed "s/ [A-Z]//g"`
			do
	...
	fi # End if [ $count_failed_check == 0 ]
	temp_line=$((i+1))
	rm -rf File_TCs_$i
done # End for i in `cat line_TCs`
