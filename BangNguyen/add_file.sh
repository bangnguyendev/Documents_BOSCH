...
#find -iname '*.c' |grep -i "$name_DataBase" |grep -i "$name_PATH" |grep -i "$name_var.c$"

file=`find |grep -i "$name_DataBase" |grep -i "$name_PATH" |grep -i "$name_var.c$" `
count=`find |grep -i "$name_DataBase" |grep -i "$name_PATH" |grep -i "$name_var.c$" -c`
...
# KIEM TRA XEM CO 1 KET QUA KHOP HAY KHONG
	...
	then
		cp -r ./hdr $name_var
		cp stub.c $name_var 
		cp `find |grep -i "$name_DataBase" |grep -i "$name_PATH" |grep -i "$name_var$"` $name_var.c	
