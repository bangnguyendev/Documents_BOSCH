...
echo -e "\e[0m"
find |grep "$name_var.c" |grep -i "$name_PATH"

file=`find |grep -i "$name_var.c" |grep -i "$name_PATH"`
count=`find |grep -i "$name_var.c" |grep -i "$name_PATH" -c`
...
# KIEM TRA XEM CO 1 KET QUA KHOP HAY KHONG
	...
	then
		cp -r ./hdr $name_var
		cp stub.c $name_var 
		cp `find |grep -i "$name_var.c" |grep -i "$name_PATH"` $name_var	
